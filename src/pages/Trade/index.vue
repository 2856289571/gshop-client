  ...
  <div class="trade-container">
    ...
    <div class="content">
      ...
      <div class="address clearFix" v-for="(userAddress,index) in userAddressList" :key="userAddress.id">
        <span class="username" :class="{selected:userAddress.isDefault==='true'?true:false}">{{userAddress.name}}</span>
        <p @click="setDefaultAddress(index)">
          <span class="s1">{{userAddress.address}}</span>
          <span class="s2">{{userAddress.phone}}</span>
          <span class="s3" v-show="userAddress.isDefault==='true'">默认地址</span>
        </p>
      </div>
      <div class="line"></div>
    ...
    <div class="trade">
      ...
      <div class="receiveInfo">
        ...
        收货人：<span>{{userAddress.name}}</span>
...
import {mapGetters, mapState} from 'vuex'
  export default {
    ...
    computed:{
      ...mapGetters(['detailArrayList','totalNum','totalAmount','tradeNo']),
      // 计算出已选地址
      userAddress(){
    ...
    methods:{
      ...
      setDefaultAddress(index){
        ...
        this.userAddressList[index].isDefault = 'true'
      },
      // 提交订单
      /* 
        1、收集订单页信息
      ...
      */
      async submitOrder(){
        let tradeInfo = {
          ...
          consignee : this.userAddress.name,
          consigneeTel : this.userAddress.phone,
          deliveryAddress : this.userAddress.address,
          paymentWay : 'ONLINE',
          orderComment : this.message,
          orderDetailList : this.detailArrayList,
        }
        try {
          const result = await this.$API.reqTradeNo(tradeInfo)
          if(result.code === 200){
            alert('创建订单成功')
            // 成功后携带订单编号跳转到下一页面
