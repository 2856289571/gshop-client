    ...
    methods:{
      async pay(){
        try {
          ...
          this.$alert(`<img src="${imgUrl}" />`, '请使用微信扫码支付', {
            ...
            beforeClose:(action,instance,done) => {
              if(action === 'confirm'){
                ...
                //   this.$message.warning('请确保你已经支付成功了')
                // }
          ...
          // 3、轮询(2s一次)发送请求，查看用户订单支付状态
          if(!this.timer){
            this.timer = setInterval(async () => {
              const result = await this.$API.reqPayStatus(this.payInfo.orderId)
